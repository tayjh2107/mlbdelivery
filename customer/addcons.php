...
<div class="about-top-grids">
	...
	<form class="myform" action="addcons.php" method="post">
		<span>Full Address of Destination</span>
		<br>
                        <textarea name="addr" maxlength="100" cols="5" style="width: 100%;" class="required"></textarea>
                    <br>
		...
		<option value="1">Normal (usually takes 5 to 7 days)</option>
		<option value="2">Express (Takes only 1 or 2 days)</option>
		<?php 
		//	$query="select * from city";
		//			$query_run=mysqli_query($con,$query);
		//				while($row = mysqli_fetch_array($query_run)){
													...
													$query_run=mysqli_query($con,$query);
														if($query_run)
															...
															if($val=mysqli_fetch_array($query_run2))
																...
																if($sent)
																	{
																		echo '<script type="text/javascript"> alert("Request added successfully, check your email") </script>';				
																	}
																else
																{
																	echo '<script type="text/javascript"> alert("Request added successfully,error: email not sent") </script>';
																}
